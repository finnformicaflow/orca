...
function ProviderPicker({ row }: { row: Row }) {
  ...
  return (
    <Select value={provider} onValueChange={(v) => setCardProvider(row, v as AgentProvider)}>
      ...
      >
        {agentLabel(provider)}{row.agentMeta?.profile && <span className="opacity-70"> · {row.agentMeta.profile}</span>}
      </SelectTrigger>
    ...
    <div className={`bg-card relative flex flex-col rounded-md border shadow-sm ${dense ? "gap-1 p-2" : "gap-2 p-3"} ${busy ? "pointer-events-none" : ""}`} aria-busy={busy}>
      ...
      {!isDone && (
        <div className={`space-y-2 border-t ${dense ? "pt-1.5" : "pt-2.5"}`}>
          {dense ? (
            // Dense folds every verb into one icon row; keep the terminal button beside the (compact)
            // Test-locally control by slotting both into the leading slot.
            <WorkstreamActions
              row={row}
              hasWork={hasWork}
              onBusy={setBusy}
